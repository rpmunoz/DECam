;cd, 'completeness/code'
sex_dir='results/sex'
psfex_dir='results/psfex'
mock_dir='results/mock'
figure_dir='results/figure'
do_debug=0

if file_test(sex_dir, /dir) EQ 0 then file_mkdir, sex_dir, /noexpand
if file_test(psfex_dir, /dir) EQ 0 then file_mkdir, psfex_dir, /noexpand
if file_test(mock_dir, /dir) EQ 0 then file_mkdir, mock_dir, /noexpand

tile_suffix=['1','2','3','4']

psf_file=psfex_dir+'/survey_cat_psfex_'+tile_suffix+'.psf'
im_file='images/survey_TILE'+tile_suffix+'_FILTERg.fits'
im_wgt_file='images/survey_TILE'+tile_suffix+'_FILTERg.WGT.fits'
im_flag_file='images/survey_TILE'+tile_suffix+'_FILTERg.FLAG.fits'

im_cat_file=sex_dir+'/survey_cat_sex_TILE'+tile_suffix+'_FILTERg.ldac'
im_check_file=sex_dir+'/survey_check_seg_sex_TILE'+tile_suffix+'_FILTERg.fits'

mock_n=10L ; Number of realizations
mock_mag_limits=[13.,20.] ; Magnitude range of simulated stars (Magnitudes are in the Vega system)
mock_mesh=[200.,200.] ; Grid to divide the image
mock_nstar=4L ; Number of artificial stars per cell
ab_vega=0. ; Magnitude conversion from Vega to AB (We add stars in the Vega system and then compute the completeness in AB mag system)

mock_coo_file=mock_dir+'/mock_coo_'+tile_suffix+'.dat'
a=byte(tile_suffix)
a_size=size(a,/dim)
b=byte(repstr(string(indgen(mock_n)+1,FORMAT='(I4)'),' ','0'))
b_size=size(b,/dim)
mock_mag_file=mock_dir+'/mock_mag_'+string(rebin(reform(a, [a_size,1]),[a_size,b_size[1]]))+'_'+string(rebin(reform(b, [b_size[0],1,b_size[1]]),[b_size[0],a_size[1],b_size[1]]))+'.dat'
mock_im_file=mock_dir+'/mock_im_'+string(rebin(reform(a, [a_size,1]),[a_size,b_size[1]]))+'_'+string(rebin(reform(b, [b_size[0],1,b_size[1]]),[b_size[0],a_size[1],b_size[1]]))+'.fits'
sex_cat_file=mock_dir+'/sex_cat_'+string(rebin(reform(a, [a_size,1]),[a_size,b_size[1]]))+'_'+string(rebin(reform(b, [b_size[0],1,b_size[1]]),[b_size[0],a_size[1],b_size[1]]))+'.ldac'

mock_completeness_file = mock_dir+'/mock_completeness_'+tile_suffix+'.dat'

im_mask_region=[{tile:'TILEX', xrange:[15535L,17759L], yrange:[16623L,19383L]}] ; In case you want to mask a bad region in the image or containing a large galaxy
im_blank_region=[{tile:'TILEY', xrange:[9000L,-1], yrange:[0L,5850L]}]
...
;	goto, mock_psf
  ...
  command='sex '+im_file[i]+' -c sex_config/ctio_decam.sex -CATALOG_NAME '+im_cat_file[i]+' -WEIGHT_TYPE MAP_WEIGHT -WEIGHT_IMAGE '+im_wgt_file[i]+' -WRITE_XML N -CHECKIMAGE_TYPE SEGMENTATION -CHECKIMAGE_NAME '+ im_check_file[i] +' -SEEING_FWHM 0.62 -BACK_SIZE 256 -BACK_FILTERSIZE 3 -DETECT_THRESH 2. -ANALYSIS_THRESH 3. -FLAG_TYPE OR -FLAG_IMAGE '+im_flag_file[i]
  print, command
  spawn, command

  mock_coo:
  im_check_data=(readfits(im_check_file[i], im_h) EQ 0)
  im_wgt_data=(readfits(im_wgt_file[i], im_h) GT 0)
  ...
  forprint, replicate(im_file[i], mock_n), replicate(im_wgt_file[i], mock_n), replicate(im_flag_file[i], mock_n), replicate(psf_file[i], mock_n), mock_mag_file[i,*], mock_im_file[i,*], sex_cat_file[i,*], FORMAT='(A,4X,A,4X,A,4X,A,4X,A,4X,A,4X,A)', textout=mock_completeness_file[i], COMMENT='#    im_file   weight_file   flag_file   psf_file   mock_mag_file   mock_im_file   sex_cat_file', /SILENT

	command='python survey_completeness.py '+mock_completeness_file[i]
	print, command
	spawn, command
    ...
    readcol, mock_mag_file[i,j], temp_x, temp_y, temp_mag, FORMAT='I,I,F', /silent
		...
		free_lun, table_u
		temp_n=n_elements(temp_x)
		create_struct, temp_cat, '', ['x','y','mag_aper','mag_auto','flags','imaflags'], 'F,F,F(10),F,I,I', dim=temp_n
		temp_cat.x=temp_x
		temp_cat.y=temp_y
	    ...
	    temp=yhist_out*1./yhist_in
	 	  plot, xhist_out+ab_vega, temp, xtitle='g (AB)', ytitle='mock_out/mock_in', xrange=[20.,24.], yrange=[0.,1.2], ystyle=1
		...
		if (j EQ 0) then begin ;OR ((j+1) mod 50 EQ 0) then begin
			...
			plot, xhist_out+1.82, yhist_out*1./yhist_in, xrange=[20.,25.], yrange=[0.,1.2]
	...
	save, mock_n, mock_nstar, mock_mesh, mock_in_x, mock_in_y, mock_out_x, mock_out_y, mock_in_mag, mock_out_mag, mock_mag_limits, ab_vega, file='survey_completeness_'+tile_suffix[i]+'_partial.sav'
	mock_in_x=0
	mock_in_y=0
	mock_in_mag=0
	mock_in_mag_match=0
	...
	p=plot(temp_x[gv], temp_y[gv], xtitle='g!Dinput!N (AB)', ytitle='Completeness', xrange=[19.9,24.], yrange=[0.,1.2], xstyle=1, ystyle=1, /current)
	p=plot([0,100], 0.9*[1,1], linestyle=2, color="red", /over)
	p=plot([0,100], 0.5*[1,1], linestyle=2, color="red", /over)
	w.save, figure_dir+'/survey_completeness_'+tile_suffix[i]+'.png', BORDER=10, RESOLUTION=300
	w.close

	w=window(dim=[800,600])
	p=plot(mock_in_mag_match[gv_out[0:20000]]+ab_vega, (mock_out_mag-mock_in_mag_match)[gv_out[0:20000]], xtitle='g!Dinput!N (AB)', ytitle='g!Doutput!N - g!Dinput!N (AB)', xrange=[19.9,24.], yrange=[-1,1], xstyle=1, ystyle=1, symbol="circle", sym_filled=1, sym_size=0.2, sym_transparency=70, linestyle="none", /current)
	p=plot([0,100], 0.*[1,1], linestyle=2, color="red", /overplot)
	w.save, figure_dir+'/survey_completeness_mag_comparison_'+tile_suffix[i]+'.png', BORDER=10, RESOLUTION=600
	w.close
	...
	; Here the code ends. The next lines are for doing extra statistics
	continue
  ...
  grid_y=make_array(grid_nx, /float, value=1.)#grid_y
		...
		for jj=0L, grid_ny-1 do begin
			...
			if n_gv_in GT 1000L AND n_gv_out GT 100L then begin
				...
				if jj EQ 5 then	begin
					plot, xhist_out+1.82, temp, xtitle='g (AB)', ytitle='mock_out/mock_in', yrange=[0.,1.2], ystyle=1
	...
	save, /variables, file='survey_completeness_'+tile_suffix[i]+'.sav'
	mock_in_x=0
	mock_in_y=0
	mock_in_mag=0
	mock_in_mag_match=0
  ...
  color_id=round(indgen(mag_levels)*255./(mag_levels-1))
	...
	w.save, 'survey_completeness_'+tile_suffix[i]+'.png', BORDER=10, RESOLUTION=300
	w.close
